...
**What you're looking at:** six pre-registered v2 experiments, each frozen before the run (PREREGISTRATION.md §10 to §15) and reproduced in [notebook 04](notebooks/04_neural_scaling.ipynb) and [notebook 05](notebooks/05_exploitation.ipynb). The figure above is the §13 exploitation result. The robustness, neural, scaling, exact-RNR, and detect-then-exploit figures are [`seed_sweep.png`](figures/seed_sweep.png), [`neural_nfsp.png`](figures/neural_nfsp.png), [`scale.png`](figures/scale.png), [`rnr_frontier.png`](figures/rnr_frontier.png), and [`dbr_frontier.png`](figures/dbr_frontier.png).

**Takeaway:** six findings, reported whichever way they fell. (i) The +256 headline is robust across 20 training seeds, not a lucky seed (§10). (ii) Neural NFSP only weakly beats tabular on small Leduc, a qualified 2/3 sample-efficiency pass after a corrected epsilon bug, and tabular still wins asymptotically (§11). (iii) At R=20, where tabular CFR cannot converge, truncated tabular CFR still beats neural NFSP (§12, an honest null). (iv) A first heads-up tilt-exploitation knob loses 169 chips/match against the tilter (§13, 95% CI [−271, −66]), because the disciplined baseline already beats it (+533) and loosening walks into its aggression; a small-sample peek had shown the opposite, and the powered run overturned it, the clearest demonstration of why pre-registration matters. (v) The reliable positive (§14): on Leduc with exact EV, a Restricted Nash Response exploits every suboptimal opponent for a strictly positive, exactly-measured gain over Nash (up to +2.16 chips per deal versus a maniac), bought with the counter's own rising exploitability ([`rnr_frontier.png`](figures/rnr_frontier.png)). §14 is the corrected, exact answer to §13 and the project's first reliable exploitation positive. (vi) The meaningful upgrade (§15): when the opponent is estimated from observed hands rather than handed over, a conservative response (p=0.5) still beats Nash for every opponent at 40 observed hands (each 95% CI excluding zero), while a raw best response to the sparsely-observed maniac loses to Nash on average, the data-biased-response lesson ([`dbr_frontier.png`](figures/dbr_frontier.png)). The pre-committed G1-and-G2 conjunction is not met (G1 fails for the maniac); G2 alone is confirmed, and that is what §15.4 reports.
