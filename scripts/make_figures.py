...
def fig_exec_summary(index):
    ...
    cap = ("EXECUTIVE SUMMARY — every headline edge as a point with its 95% "
           ...
           "honest takeaway: the two RL edges directionally positive and the two "
...
def fig_pool(index):
    ...
    verdict = ("tops the static leaderboard" if rank == 1 else
               ...
               "objective)")
    cap_sw = (f"Static sweep: the same RL generalist dropped into a round-robin of static "
    ...
    wr = ", ".join(f"{n} {sum(1 for v in nets[n] if v > 0) / len(nets[n]):.0%}"
                   for n in order)
    cap_box = (f"Pool: per-match net chips per agent — each dot is one held-out match, the "
...
def fig_icm(index):
    ...
    for ladder, lrows in sorted(by_ladder.items(), key=lambda kv: kv[0],
        ...
        cap = (f"ICM edge ({npl}-player, ladder {label}): per-init-seed "
               ...
               f"seeds ICM>chips; {ci_note}). The earlier small-positive estimate "
               f"came from 3 noisier unseeded inits; {tail}.")
...
def fig_block_b(index):
    ...
    if rows:
        ...
        cap = ("B3: widening the multi-hand bust clip does NOT help — the "
    ...
    if rows:
        cap = ("B4: snapshot self-play vs the fixed-vs-myopic recipe, mean "
...
def fig_tilt_realdata(index):
    ...
    cap = (f"Real-data tilt validation ({d['n_sequences']:,} sessions, "
           ...
           f"{regime_txt} Honest scale: the shifts are small (1-3pp) in absolute "
...
def fig_rollout_fe(index):
    ...
    cap = (f"B1: warmed-belief rollout fold-equity, net chips of the "
