...
| **Detect-then-exploit on Leduc** ([pre-registered §15](PREREGISTRATION.md), exact EV vs an estimated opponent) | conservative p=0.5 gains **+0.74 / +1.31 / +1.13 / +0.78** over Nash at 40 observed hands (station / maniac / uniform / loose-passive) | 95% CI excludes 0 (all four) | **G2 confirmed, opponent estimated not handed**: play Nash, observe N hands, then RNR on the estimate beats Nash with a conservative p (the meaningful upgrade over §14). The pre-committed G1-and-G2 conjunction is NOT met: G1 (large-N recovers the exact ceiling) fails for the maniac, where a raw best response (p=1) loses to Nash. G2 alone is confirmed; that failure is the data-biased-response lesson |
